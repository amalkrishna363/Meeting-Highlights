  ...
  return (
    <div style={{
      ...
      <div style={{ textAlign: "center", marginBottom: "40px" }}>
        ...
        <p style={{ fontSize: "13px", color: "rgba(255,255,255,0.35)", fontWeight: 400 }}>Action Log AI is working its magic...</p>
      </div>
        ...
        {steps.map((step, i) => {
          ...
          return (
            <div key={i} style={{
              ...
              background: isActive ? "rgba(99,102,241,0.15)" : isDone ? "rgba(255,255,255,0.03)" : "rgba(255,255,255,0.02)",
              border: isActive ? "1px solid rgba(99,102,241,0.4)" : "1px solid rgba(255,255,255,0.06)",
              backdropFilter: "blur(12px)",
              boxShadow: isActive ? "0 4px 24px rgba(99,102,241,0.2)" : "none",
              transition: "all 0.4s ease",
            }}>
              <div style={{
                width: "36px", height: "36px", borderRadius: "10px", flexShrink: 0,
                background: isDone ? "linear-gradient(135deg, #6366f1, #8b5cf6)" : isActive ? `${step.color}22` : "rgba(255,255,255,0.04)",
                border: `1px solid ${isActive ? step.color + "66" : isDone ? "transparent" : "rgba(255,255,255,0.08)"}`,
                display: "flex", alignItems: "center", justifyContent: "center",
                fontSize: "16px",
                boxShadow: isDone ? "0 4px 12px rgba(99,102,241,0.35)" : "none",
                transition: "all 0.4s ease",
              }}>
                {isDone ? "✓" : step.icon}
              </div>
              <span style={{ fontSize: "13px", fontWeight: isActive ? 700 : 500, color: isActive ? "#ffffff" : isDone ? "rgba(255,255,255,0.5)" : "rgba(255,255,255,0.25)", flex: 1, transition: "all 0.3s" }}>
                {step.label}
              </span>
              {isActive && (
                <div style={{ display: "flex", gap: "3px" }}>
                  {[0,1,2].map(d => <div key={d} style={{ width: "5px", height: "5px", borderRadius: "50%", background: "#818cf8", animation: "pulse 1s infinite", animationDelay: `${d * 0.2}s` }} />)}
                </div>
              )}
...
const ResultTabs = ({ result, activeTab, setActiveTab }) => (
  <div>
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "16px", marginBottom: "28px" }}>
      {[
        { label: "Words", value: result.transcript?.split(" ").length || 0, icon: "📝", color: "#6366f1" },
        { label: "Summary", value: "Done", icon: "📋", color: "#8b5cf6" },
        { label: "Actions", value: result.action_items?.length || 0, icon: "✅", color: "#a78bfa" },
      ].map((stat, i) => (
      ...
      {tabs.map((tab) => (
        <button key={tab.id} onClick={() => setActiveTab(tab.id)} style={{
          ...
          {tab.id === "actions" && result.action_items?.length > 0 && (
            <span style={{ background: activeTab === tab.id ? "rgba(99,102,241,0.3)" : "rgba(255,255,255,0.08)", color: activeTab === tab.id ? "#a5b4fc" : "rgba(255,255,255,0.4)", borderRadius: "999px", padding: "1px 8px", fontSize: "11px", fontWeight: 700 }}>{result.action_items.length}</span>
          )}
        </button>
      ))}
    </div>
...
function Home() {
  ...
  const handleDownloadPDF = () => {
    ...
    doc.setFontSize(18); doc.setFont("helvetica", "bold"); doc.setTextColor(15, 23, 42); doc.text("Action Log — Meeting Highlights", margin, y); y += 6;
  ...
  return (
    <Layout>
      {loading && <LoadingOverlay steps={[{ label: "Transcribing audio", icon: "🎧", color: "#6366f1" }, { label: "Generating summary", icon: "📋", color: "#8b5cf6" }, { label: "Extracting action items", icon: "✅", color: "#a78bfa" }]} />}
        ...
        <div style={{ maxWidth: "580px" }}>
          <div style={{ marginBottom: "36px" }}>
            <div style={{ display: "inline-flex", alignItems: "center", gap: "8px", background: "rgba(99,102,241,0.12)", border: "1px solid rgba(99,102,241,0.25)", borderRadius: "999px", padding: "6px 16px", marginBottom: "20px" }}>
              ...
              <span style={{ fontSize: "11px", color: "#a5b4fc", fontWeight: 700, letterSpacing: "1px" }}>AI POWERED</span>
            </div>
            <h1 style={{ fontSize: "36px", fontWeight: 800, color: "#ffffff", letterSpacing: "-1px", marginBottom: "10px", lineHeight: 1.15 }}>
              ...
              <span style={{ background: "linear-gradient(135deg, #818cf8, #a78bfa)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>audio recording</span>
            </h1>
            <p style={{ color: "rgba(255,255,255,0.45)", fontSize: "15px", lineHeight: 1.6 }}>Get a full transcript, smart summary, and action items in seconds.</p>
          </div>
          ...
          <div style={{ ...glass, padding: "36px" }}>
            <label
              ...
              {file ? (
                <><p style={{ color: "#a5b4fc", fontWeight: 700, fontSize: "15px", marginBottom: "4px" }}>{file.name}</p><p style={{ color: "rgba(255,255,255,0.3)", fontSize: "12px" }}>{(file.size / 1024 / 1024).toFixed(2)} MB · Ready to process</p></>
              ) : (
                <><p style={{ color: "rgba(255,255,255,0.7)", fontWeight: 700, fontSize: "15px", marginBottom: "6px" }}>Drop your audio file here</p><p style={{ color: "rgba(255,255,255,0.3)", fontSize: "13px" }}>MP3, M4A, WAV, OGG · or click to browse</p></>
              )}
            </label>
            <button onClick={handleUpload} disabled={!file || loading} style={{
              width: "100%", padding: "15px", borderRadius: "14px", border: "none",
              fontSize: "15px", fontWeight: 700, cursor: file && !loading ? "pointer" : "not-allowed",
              background: file && !loading ? "linear-gradient(135deg, #6366f1, #8b5cf6)" : "rgba(255,255,255,0.05)",
              color: file && !loading ? "#fff" : "rgba(255,255,255,0.2)",
              boxShadow: file && !loading ? "0 8px 32px rgba(99,102,241,0.4)" : "none",
        ...
        <div style={{ maxWidth: "720px" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "28px" }}>
            <div>
              ...
              <p style={{ color: "rgba(255,255,255,0.35)", fontSize: "13px" }}>{result.filename}</p>
            </div>
            <div style={{ display: "flex", gap: "10px" }}>
              ...
              <button onClick={() => { setFile(null); setResult(null); }} style={{ ...glass, border: "1px solid rgba(255,255,255,0.1)", borderRadius: "12px", padding: "10px 20px", fontSize: "13px", fontWeight: 600, color: "rgba(255,255,255,0.5)", cursor: "pointer" }}>↩ New</button>
            </div>
          </div>
